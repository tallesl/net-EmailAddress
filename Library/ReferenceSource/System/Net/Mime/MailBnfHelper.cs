...
namespace EmailAddressValidation.ReferenceSource.System.Net.Mime
{
    internal static class MailBnfHelper
    {
        ...
        internal static bool SkipCFWS(string data, ref int offset)
        {
            ...
            {
                if (data[offset] > 127)
                    throw new FormatException(string.Format(SR.MailHeaderFieldInvalidCharacter, data[offset]));
        ...
        internal static void ValidateHeaderName(string data)
        {
            ...
            for (; offset < data.Length; offset++)
            {
                if (data[offset] > Ftext.Length || !Ftext[data[offset]])
                    throw new FormatException(string.Format(SR.InvalidHeaderName));
            }
            if (offset == 0)
                throw new FormatException(string.Format(SR.InvalidHeaderName));
        }
        ...
        internal static string ReadQuotedString(string data, ref int offset, StringBuilder builder, bool doesntRequireQuotes, bool permitUnicodeInDisplayName)
        {
            ...
            throw new FormatException(string.Format(SR.MailHeaderFieldMalformedHeader));
        }
        ...
        private static bool CheckForUnicode(char ch, bool allowUnicode)
        {
            ...
            if (!allowUnicode)
            {
                throw new FormatException(string.Format(SR.MailHeaderFieldInvalidCharacter, ch));
            }
            return true;
        }
